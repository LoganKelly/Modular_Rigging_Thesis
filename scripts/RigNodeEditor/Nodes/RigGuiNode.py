# RigGuiNode extends the BaseNode class
...
class RigGuiNode(BaseNode,QtSvg.QGraphicsSvgItem):
    ...
    def mousePressEvent(self, event):
        super(RigGuiNode, self).mousePressEvent(event)
    ...
    def paint(self, painter, option, widget):
        ...
        super(RigGuiNode, self).paint(painter, option, widget)
    ...
    def addConnections(self):
        self.inputConnection = ConnectRigNode(":/rigInputConnection.svg", self)
        self.inputConnection.setPos(self.width()/2 - self.inputConnection.width()/2 + 6, -self.inputConnection.height()+6.0)
    ...
    def setupWidgetFromElem(self,elem):
        ...
        widgetMenu.rigName.setText(self.name)
        version = elem.get('version')
        widgetMenu.versionNumLabel.setText(version)
        
    def getXMLStart(self):
        return "Rig XML Started, metaNodeName = "+self.metaNodeName
    
    def getXMLEnd(self):
        return "Rig XML Ended"
    
    def getXMLElem(self,forSave):
        widgetMenu = self.getWidgetMenu()
        attributes = {}
        attributes['name']= widgetMenu.rigName.text()
        if forSave:
        ...
        updateElem = xml.Element('rig',attributes)
        return updateElem